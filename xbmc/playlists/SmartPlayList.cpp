...
std::string CSmartPlaylistRule::GetBooleanQuery(const std::string &negate, const std::string &strType) const
{
  if (strType == "movies")
  {
    if (m_field == FieldInProgress)
      return "movie_view.idFile " + negate + " IN (SELECT DISTINCT idFile FROM bookmark WHERE type = 1 AND bookmark.idViewer ="+CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID+")";
  ...
  {
    if (m_field == FieldInProgress)
      return "episode_view.idFile " + negate + " IN (SELECT DISTINCT idFile FROM bookmark WHERE type = 1AND bookmark.idViewer ="+CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID+")";
  }
