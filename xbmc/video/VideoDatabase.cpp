...
void CVideoDatabase::generateViewsPerUser() {
		...
		if (!g_advancedSettings.Initialized() || CVideoDatabase::createdViewPerUser || GetDBVersion()< GetSchemaVersion())
			return;
...
//		CLog::Log(LOGINFO, "%s", seasonview);
		...
		std::string movieview = PrepareSQL("CREATE VIEW movie_view" + g_advancedSettings.m_databaseVideo.userID + " AS SELECT"
			"  movie.*,"
			"  sets.strSet AS strSet,"
			"  sets.strOverview AS strSetOverview,"
			"  files.strFileName AS strFileName,"
  ...
  try
  {
    ...
    { // query DB for any movies matching online id and year
      std::string strSQL = PrepareSQL("SELECT watchlist.playCount, watchlist.lastPlayed "
									  ...
									  g_advancedSettings.m_databaseVideo.userID.c_str(), details.GetUniqueID().c_str(), details.GetYear(), idMovie);
      m_pDS->query(strSQL);
  ...
  try
  {
    ...
    if (CProfilesManager::GetInstance().GetMasterProfile().getLockMode() != LOCK_MODE_EVERYONE && !g_passwordManager.bMasterUser)
    {
      ...
      else if (idContent == VIDEODB_CONTENT_MUSICVIDEOS)
      {
        ...
        extFilter.AppendJoin("join files on files.idFile = musicvideo_view.idFile join path on files.idPath = path.idPath  LEFT JOIN watchlist ON watchlist.idFile=movie_view.idFile AND watchlist.idViewer = " + g_advancedSettings.m_databaseVideo.userID);
      }
      else
        return false;
    }
    else
    {
      ...
      else if (idContent == VIDEODB_CONTENT_MUSICVIDEOS)
      {
        ...
        extFilter.AppendJoin("join files on files.idFile = musicvideo_view.idFile LEFT JOIN watchlist ON watchlist.idFile=movie_view.idFile AND watchlist.idViewer = " + g_advancedSettings.m_databaseVideo.userID);
