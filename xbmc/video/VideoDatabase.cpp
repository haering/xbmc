...
std::string CVideoDatabase::musicVideoView;
std::string CVideoDatabase::tvShowView;
std::string CVideoDatabase::episodeView;
std::string CVideoDatabase::movieView;
std::string CVideoDatabase::tvshowcountsView;
std::string CVideoDatabase::seasonView;
bool CVideoDatabase::initialized = false;
std::string CVideoDatabase::userID = "none";
bool CVideoDatabase::createdViewPerUser=false;
//********************************************************************************************************************************
...
{
	//std::cout << "Open aufruf " << CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID << std::endl;
	//CLog::Log(LOGINFO, "Open aufruf %s %s %d", CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID.c_str(), CVideoDatabase::userID.c_str(), CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID.compare(CVideoDatabase::userID));
	if ((CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->Initialized() && !CVideoDatabase::initialized ) || CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID.compare(CVideoDatabase::userID) != 0) {
//		movieView = "movieview LEFT JOIN watchlist ON watchlist.idFile=movieview.idFile AND watchlist.idViewer = " +  CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID  ;
//		musicVideoView = "musicvideoview LEFT JOIN watchlist ON watchlist.idFile=musicvideoview.idFile AND watchlist.idViewer = "+ CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID;
		//std::cout << "Mal wieder Open2" << std::endl;
		//CLog::Log(LOGINFO, "Open drinnen");
		CVideoDatabase::tvShowView = "tvshow_view"+ CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID+" tvshow_view ";
		CVideoDatabase::movieView = "movie_view"+ CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID+" movie_view ";
		CVideoDatabase::musicVideoView = "musicvideo_view"+ CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID+" musicvideo_view ";
		CVideoDatabase::episodeView = "episode_view" + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID+" episode_view ";
		CVideoDatabase::tvshowcountsView = "tvshowcounts"+ CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID+ " tvshowcounts ";
		CVideoDatabase::initialized = true;
		CVideoDatabase::createdViewPerUser = false;
		CVideoDatabase::userID = CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID;
...
void CVideoDatabase::CreateTables()
{
  ...
  m_pDS->exec("CREATE TABLE bookmark ( idBookmark integer AUTO_INCREMENT , idViewer integer, idFile integer, timeInSeconds double, totalTimeInSeconds double, thumbNailImage text, player text, playerState text, type integer, PRIMARY KEY(idBookmark,idViewer))\n");
  ...
  m_pDS->exec("CREATE TABLE files ( idFile integer primary key, idPath integer, strFilename text, dateAdded text)");
   ...
   CLog::Log(LOGINFO, "create watchlist table");
	m_pDS->exec("CREATE TABLE watchlist (  idFile integer, idViewer integer, playCount integer,  lastPlayed text, PRIMARY KEY (idFile,idViewer))");
...
void CVideoDatabase::CreateAnalytics()
{
  ...
  m_pDS->exec("CREATE TRIGGER delete_file AFTER DELETE ON files FOR EACH ROW BEGIN "
			  ...
			  "DELETE FROM watchlist WHERE idFile = old.idFile;"
              "END");
...
void CVideoDatabase::generateViewsPerUser() {

		if (!CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->Initialized() || CVideoDatabase::createdViewPerUser || GetDBVersion()< GetSchemaVersion())
			return;
		CVideoDatabase::createdViewPerUser = true;

		CVideoDatabase::tvShowView = "tvshow_view" + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID + " tvshow_view ";
		CVideoDatabase::movieView = "movie_view" + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID + " movie_view ";
		CVideoDatabase::musicVideoView = "musicvideo_view" + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID + " musicvideo_view ";
		CVideoDatabase::episodeView = "episode_view" + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID + " episode_view ";
		CVideoDatabase::seasonView = "season_view" + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID + " season_view ";
		CVideoDatabase::tvshowcountsView = "tvshowcounts" + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID + " tvshowcounts ";


		CLog::Log(LOGINFO, "create tvshowcounts");
		m_pDS->exec("DROP VIEW IF EXISTS tvshowcounts" + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID);


		std::string tvshowcounts = PrepareSQL("CREATE VIEW tvshowcounts" + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID + " AS SELECT "
			"      tvshow.idShow AS idShow,"
			"      MAX(watchlist.lastPlayed) AS lastPlayed,"
			"      NULLIF(COUNT(episode.c12), 0) AS totalCount,"
			"      COUNT(watchlist.playCount) AS watchedcount,"
			"      NULLIF(COUNT(DISTINCT(episode.c12)), 0) AS totalSeasons, "
			"      MAX(files.dateAdded) as dateAdded "
			"    FROM tvshow"
			"      LEFT JOIN episode ON"
			...
			"   watchlist.idFile=episode.idFile AND watchlist.idViewer =  " + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID +
			" GROUP BY tvshow.idShow");
		m_pDS->exec(tvshowcounts);
		...
		CLog::Log(LOGINFO, "Drop tvshow_view");
		m_pDS->exec("DROP VIEW IF EXISTS tvshow_view" + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID);
		CLog::Log(LOGINFO, "create tvshow_view");
		std::string tvshowview =
			PrepareSQL("CREATE VIEW tvshow_view"
			...
			"  LEFT JOIN path ON"
			"    path.idPath=tvshowlinkpath_minview.idPath"
			" INNER JOIN " + CVideoDatabase::tvshowcountsView + " ON"
			" tvshow.idShow = tvshowcounts.idShow "
			"  LEFT JOIN rating ON"
			"    rating.rating_id=tvshow.c%02d "
			"  LEFT JOIN uniqueid ON"
		...
		CLog::Log(LOGINFO, "create episode_view");
		m_pDS->exec("DROP VIEW IF EXISTS episode_view" + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID);
		std::string episodeview = PrepareSQL("CREATE  VIEW episode_view" + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID + " AS SELECT "
			"  episode.*,"
			"  files.strFileName AS strFileName,"
			"  path.strPath AS strPath,"
			"  watchlist.playCount AS playCount,"
			"  watchlist.lastPlayed AS lastPlayed,"
			"  files.dateAdded AS dateAdded,"
			"  tvshow.c%02d AS strTitle,"
			"  tvshow.c%02d AS genre,"
			"  tvshow.c%02d AS studio,"
			...
			VIDEODB_ID_TV_TITLE, VIDEODB_ID_TV_GENRE,
			VIDEODB_ID_TV_STUDIOS, VIDEODB_ID_TV_PREMIERED,
			VIDEODB_ID_TV_MPAA, CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID.c_str(), CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID.c_str(),
			VIDEODB_ID_EPISODE_RATING_ID, VIDEODB_ID_EPISODE_IDENT_ID);
		m_pDS->exec(episodeview);

		CLog::Log(LOGINFO, "create seasonview");
		m_pDS->exec("DROP VIEW IF EXISTS season_view" + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID);
		std::string seasonview = PrepareSQL("CREATE VIEW season_view%s AS SELECT "
            "  seasons.idSeason AS idSeason,"
            "  seasons.idShow AS idShow,"
            "  seasons.season AS season,"
            "  seasons.name AS name,"
			...
			"  count(watchlist.playCount) AS playCount,"
			...
			" LEFT JOIN watchlist ON"
			" watchlist.idFile=files.idFile AND watchlist.idViewer = %s "
            "GROUP BY seasons.idSeason,"
            "         seasons.idShow,"
            "         seasons.season,"
            "         seasons.name,"
            ...
            "         tvshow_view.c%02d ",
                               CServiceBroker::GetSettingsComponent()
                                   ->GetAdvancedSettings()
                                   ->m_databaseVideo.userID.c_str(),
			VIDEODB_ID_TV_TITLE, VIDEODB_ID_TV_PLOT, VIDEODB_ID_TV_PREMIERED,
			VIDEODB_ID_TV_GENRE, VIDEODB_ID_TV_STUDIOS, VIDEODB_ID_TV_MPAA,
			VIDEODB_ID_EPISODE_AIRED, CVideoDatabase::tvShowView.c_str(), CVideoDatabase::episodeView.c_str(),
			VIDEODB_ID_EPISODE_SEASON, CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID.c_str(),
            VIDEODB_ID_TV_TITLE, VIDEODB_ID_TV_PLOT, VIDEODB_ID_TV_PREMIERED,
            VIDEODB_ID_TV_GENRE, VIDEODB_ID_TV_STUDIOS, VIDEODB_ID_TV_MPAA);
                //		CLog::Log(LOGINFO, "%s", seasonview);
		m_pDS->exec(seasonview);

		CLog::Log(LOGINFO, "create musicvideo_view");
		m_pDS->exec("DROP VIEW IF EXISTS musicvideo_view" + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID);
		m_pDS->exec("CREATE VIEW musicvideo_view" + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID + " AS SELECT"
			"  musicvideo.*,"
			"  files.strFileName as strFileName,"
			"  path.strPath as strPath,"
			"  watchlist.playCount as playCount,"
			...
			"    path.idPath=files.idPath"
			" LEFT JOIN watchlist ON"
			"    watchlist.idFile=musicvideo.idFile AND watchlist.idViewer = " + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID +
			"  LEFT JOIN bookmark ON"
			"    bookmark.idFile=musicvideo.idFile AND bookmark.type=1  AND bookmark.idViewer = " + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID );

		CLog::Log(LOGINFO, "create movie_view");
		m_pDS->exec("DROP VIEW IF EXISTS movie_view" + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID);
		std::string movieview = PrepareSQL("CREATE VIEW movie_view" + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID + " AS SELECT"
			"  movie.*,"
			"  sets.strSet AS strSet,"
			"  sets.strOverview AS strSetOverview,"
			"  files.strFileName AS strFileName,"
			...
			"    path.idPath=files.idPath"
			" LEFT JOIN watchlist ON"
			"    watchlist.idFile=movie.idFile AND watchlist.idViewer = " + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID +
			"  LEFT JOIN bookmark ON"
			"    bookmark.idFile=movie.idFile AND bookmark.type=1 AND bookmark.idViewer = " + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID +
			"  LEFT JOIN rating ON"
			"    rating.rating_id=movie.c%02d"
			"  LEFT JOIN uniqueid ON"
			"    uniqueid.uniqueid_id=movie.c%02d",
...
bool CVideoDatabase::GetPathsForTvShow(int idShow, std::set<int>& paths)
{
  ...
  try
  {
    ...
    // add base path
    strSQL = PrepareSQL("SELECT strPath FROM "+CVideoDatabase::tvShowView+" WHERE idShow=%i", idShow);
...
void CVideoDatabase::GetMusicVideosByArtist(const std::string& strArtist, CFileItemList& items)
{
  try
  {
    ...
    if (strArtist.empty())  // TODO: SMARTPLAYLISTS what is this here for???
      strSQL=PrepareSQL("select distinct * from " + CVideoDatabase::musicVideoView + " join actor_link on actor_link.media_id=musicvideo_view.idMVideo AND actor_link.media_type='musicvideo' join actor on actor.actor_id=actor_link.actor_id");
    else
      strSQL=PrepareSQL("select * from " + CVideoDatabase::musicVideoView + " join actor_link on actor_link.media_id=musicvideo_view.idMVideo AND actor_link.media_type='musicvideo' join actor on actor.actor_id=actor_link.actor_id where actor.name='%s'", strArtist.c_str());
    m_pDS->query( strSQL );
...
bool CVideoDatabase::GetMovieInfo(const std::string& strFilenameAndPath, CVideoInfoTag& details, int idMovie /* = -1 */, int getDetails /* = VideoDbDetailsAll */)
{
  try
  {
    ...
    std::string sql = PrepareSQL("select * from " + CVideoDatabase::movieView + " where idMovie=%i", idMovie);
    if (!m_pDS->query(sql))
      return false;
...
bool CVideoDatabase::GetTvShowInfo(const std::string& strPath, CVideoInfoTag& details, int idTvShow /* = -1 */, CFileItem *item /* = NULL */, int getDetails /* = VideoDbDetailsAll */)
{
  try
  {
    ...
    std::string sql = PrepareSQL("SELECT * FROM " + CVideoDatabase::tvShowView + " WHERE idShow=%i GROUP BY idShow", idTvShow);
    if (!m_pDS->query(sql))
      return false;
...
bool CVideoDatabase::GetEpisodeInfo(const std::string& strFilenameAndPath, CVideoInfoTag& details, int idEpisode /* = -1 */, int getDetails /* = VideoDbDetailsAll */)
{
  try
  {
    ...
    std::string sql = PrepareSQL("select * from " + CVideoDatabase::episodeView + " where idEpisode=%i",idEpisode);
    if (!m_pDS->query(sql))
      return false;
...
bool CVideoDatabase::GetMusicVideoInfo(const std::string& strFilenameAndPath, CVideoInfoTag& details, int idMVideo /* = -1 */, int getDetails /* = VideoDbDetailsAll */)
{
  try
  {
	...
	std::string sql = PrepareSQL("select * from %s where idMVideo=%i", CVideoDatabase::musicVideoView.c_str(), idMVideo);
    if (!m_pDS->query(sql))
      return false;
...
bool CVideoDatabase::GetFileInfo(const std::string& strFilenameAndPath, CVideoInfoTag& details, int idFile /* = -1 */)
{
  try
  {
    ...
    std::string sql = PrepareSQL("SELECT * FROM files "
                                ...
                                "WHERE files.idFile = %i", CBookmark::RESUME, CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID.c_str(), CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID.c_str(), idFile);
    if (!m_pDS->query(sql))
      return false;
    ...
    details.SetPlayCount( std::max(details.GetPlayCount(), m_pDS->fv("watchlist.playCount").get_asInt()));
    if (!details.m_lastPlayed.IsValid())
      details.m_lastPlayed.SetFromDBDateTime(m_pDS->fv("watchlist.lastPlayed").get_asString());
    ...
    const std::map<std::string, std::string> &artwork, int idMovie /* = -1 */)
{
  try
  {
    ...
    { // query DB for any movies matching online id and year
      std::string strSQL = PrepareSQL("SELECT watchlist.playCount, watchlist.lastPlayed "
                                      "FROM movie "
                                      "  INNER JOIN files "
                                      ...
                                      "WHERE movie.premiered LIKE '%i%%' AND movie.idMovie!=%i AND watchlist.playCount > 0", 
									  CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID.c_str(), details.GetUniqueID().c_str(), details.GetYear(), idMovie);
      m_pDS->query(strSQL);
      ...
      if (!m_pDS->eof())
      {
		...
		strSQL = PrepareSQL("update watchlist set playCount=%i,lastPlayed='%s' where idFile=%i AND watchlist.idViewer = %s ", playCount, lastPlayed.GetAsDBDateTime().c_str(), idFile, CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID.c_str());
        m_pDS->exec(strSQL);
      }
    ...
    const std::map<std::string, std::string> &artwork, int idShow, int idEpisode)
{
  try
  {
    ...
    { // query DB for any episodes matching idShow, Season and Episode
      std::string strSQL = PrepareSQL("SELECT watchlist.playCount, watchlist.lastPlayed "
                                      "FROM episode INNER JOIN files ON files.idFile=episode.idFile LEFT JOIN watchlist ON watchlist.idFile=files.idFile AND watchlist.idViewer = %s "
                                      "WHERE episode.c%02d=%i AND episode.c%02d=%i AND episode.idShow=%i "
									  "AND episode.idEpisode!=%i AND watchlist.playCount > 0", CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID.c_str(),
...
void CVideoDatabase::GetBookMarksForFile(const std::string& strFilenameAndPath, VECBOOKMARKS& bookmarks, CBookmark::EType type /*= CBookmark::STANDARD*/, bool bAppend, long partNumber)
{
  try
  {
    ...
    else
    {
	  ...
	  std::string strSQL = PrepareSQL("select * from bookmark where idFile=%i and type=%i and idViewer = %s  order by timeInSeconds", idFile, (int)type, CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID.c_str());
      m_pDS->query( strSQL );
      while (!m_pDS->eof())
      {
        CBookmark bookmark;
...
void CVideoDatabase::DeleteResumeBookMark(const CFileItem& item)
{
  ...
  try
  {
    std::string sql = PrepareSQL("delete from bookmark where idFile=%i and type=%i AND idViewer = %s", fileID, CBookmark::RESUME, CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID.c_str());
    m_pDS->exec(sql);
...
void CVideoDatabase::GetEpisodesByFile(const std::string& strFilenameAndPath, std::vector<CVideoInfoTag>& episodes)
{
  try
  {
    std::string strSQL = PrepareSQL("select * from %s where idFile=%i order by c%02d, c%02d asc", CVideoDatabase::episodeView.c_str(), GetFileId(strFilenameAndPath), VIDEODB_ID_EPISODE_SORTSEASON, VIDEODB_ID_EPISODE_SORTEPISODE);
    m_pDS->query(strSQL);
    while (!m_pDS->eof())
    {
...
void CVideoDatabase::AddBookMarkToFile(const std::string& strFilenameAndPath, const CBookmark &bookmark, CBookmark::EType type /*= CBookmark::STANDARD*/)
{
  try
  {
    ...
    {
      strSQL=PrepareSQL("select idBookmark from bookmark where idFile=%i and type=1 and idViewer ="+CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID, idFile);
    }
    else if (type == CBookmark::STANDARD) // get the same bookmark again, and update. not sure here as a dvd can have same time in multiple places, state will differ thou
    {
      ...
      strSQL=PrepareSQL("select idBookmark from bookmark where idFile=%i and type=%i and (timeInSeconds between %f and %f) and playerState='%s' and idViewer ="+CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID, idFile, (int)type, mintime, maxtime, bookmark.playerState.c_str());
    }
    ...
    if (idBookmark >= 0 )
      strSQL=PrepareSQL("update bookmark set timeInSeconds = %f, totalTimeInSeconds = %f, thumbNailImage = '%s', player = '%s', playerState = '%s' where idBookmark = %i AND idViewer = "+CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID, bookmark.timeInSeconds, bookmark.totalTimeInSeconds, bookmark.thumbNailImage.c_str(), bookmark.player.c_str(), bookmark.playerState.c_str(), idBookmark);
    else
      strSQL=PrepareSQL("insert into bookmark (idBookmark, idFile,idViewer, timeInSeconds, totalTimeInSeconds, thumbNailImage, player, playerState, type) values(NULL,%i,"+CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID+",%f,%f,'%s','%s','%s', %i)", idFile, bookmark.timeInSeconds, bookmark.totalTimeInSeconds, bookmark.thumbNailImage.c_str(), bookmark.player.c_str(), bookmark.playerState.c_str(), (int)type);
...
void CVideoDatabase::ClearBookMarkOfFile(const std::string& strFilenameAndPath, CBookmark& bookmark, CBookmark::EType type /*= CBookmark::STANDARD*/)
{
  try
  {
	...
	std::string strSQL = PrepareSQL("select idBookmark from bookmark where idFile=%i and type=%i and playerState like '%s' and player like '%s' and (timeInSeconds between %f and %f) AND idViewer = %s", idFile, type, bookmark.playerState.c_str(), bookmark.player.c_str(), mintime, maxtime, CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID.c_str());

    m_pDS->query( strSQL );
    if (m_pDS->num_rows() != 0)
    {
      int idBookmark = m_pDS->get_field_value("idBookmark").get_asInt();
	  strSQL = PrepareSQL("delete from bookmark where idBookmark=%i AND idViewer = %s ", idBookmark, CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID.c_str());
      m_pDS->exec(strSQL);
      if (type == CBookmark::EPISODE)
      {
...
void CVideoDatabase::ClearBookMarksOfFile(int idFile, CBookmark::EType type /*= CBookmark::STANDARD*/)
{
  ...
  try
  {
    ...
    std::string strSQL=PrepareSQL("delete from bookmark where idFile=%i and type=%i AND idViewer= %s", idFile, (int)type, CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID.c_str());
    m_pDS->exec(strSQL);
    if (type == CBookmark::EPISODE)
    {
...
bool CVideoDatabase::GetBookMarkForEpisode(const CVideoInfoTag& tag, CBookmark& bookmark)
{
  try
  {
    std::string strSQL = PrepareSQL("select bookmark.* from bookmark join episode on episode.c%02d=bookmark.idBookmark where episode.idEpisode=%i AND bookmark.idViewer =  %s", VIDEODB_ID_EPISODE_BOOKMARK, tag.m_iDbId, CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID.c_str());
    m_pDS->query( strSQL );
    if (!m_pDS->eof())
    {
...
void CVideoDatabase::AddBookMarkForEpisode(const CVideoInfoTag& tag, const CBookmark& bookmark)
{
  try
  {
	...
	std::string strSQL = PrepareSQL("delete from bookmark where idBookmark in (select c%02d from episode where c%02d=%i and c%02d=%i and idFile=%i) AND bookmark.idViewer = %s", VIDEODB_ID_EPISODE_BOOKMARK, VIDEODB_ID_EPISODE_SEASON, tag.m_iSeason, VIDEODB_ID_EPISODE_EPISODE, tag.m_iEpisode, idFile, CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID.c_str());
    m_pDS->exec(strSQL);
...
void CVideoDatabase::DeleteBookMarkForEpisode(const CVideoInfoTag& tag)
{
  try
  {
	  std::string strSQL = PrepareSQL("delete from bookmark where idBookmark in (select c%02d from episode where idEpisode=%i) AND bookmark.idViewer = %s ", VIDEODB_ID_EPISODE_BOOKMARK, tag.m_iDbId, CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID.c_str());
    m_pDS->exec(strSQL);
...
bool CVideoDatabase::GetResumePoint(CVideoInfoTag& tag)
{
  ...
  try
  {
    ...
    {
		std::string strSQL = PrepareSQL("select timeInSeconds, totalTimeInSeconds from bookmark where idFile=%i and type=%i AND idViewer = %s  order by timeInSeconds", tag.m_iFileId, CBookmark::RESUME, CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID.c_str());
      m_pDS2->query( strSQL );
      if (!m_pDS2->eof())
      {
...
void CVideoDatabase::SetVideoSettings(int idFile, const CVideoSettings &setting)
{
  try
  {
    ...
    { // add the items
      m_pDS->close();
      strSQL= "INSERT INTO settings (idFile,userID, Deinterlace,ViewMode,ZoomAmount,PixelRatio, VerticalShift, "
      ...
      strSQL += PrepareSQL("(%i,%s,%i,%f,%f,%f,%i,%i,%f,%i,%f,%f,%f,%f,%f,%i,%f,%f,%i,%i,%i,%i,%i,%i,%i,%f,%i,%i)",
                        ...
                        setting.m_Contrast, setting.m_Gamma, setting.m_VolumeAmplification, setting.m_AudioDelay, setting.m_ResumeTime,
                        setting.m_Sharpness, setting.m_NoiseReduction, setting.m_CustomNonLinStretch, setting.m_PostProcess, setting.m_ScalingMethod,
...
void CVideoDatabase::UpdateTables(int iVersion)
{
  ...
  if (iVersion < 113)
  {
    m_pDS->exec("ALTER TABLE settings ADD userID integer");
...
bool CVideoDatabase::GetPlayCounts(const std::string &strPath, CFileItemList &items)
{
  ...
  try
  {
	...
	std::string sql =
		"SELECT"
		"  files.strFilename, watchlist.playCount,"
		"  bookmark.timeInSeconds, bookmark.totalTimeInSeconds "
		"FROM files"
		"  LEFT JOIN bookmark ON"
		"    files.idFile = bookmark.idFile AND bookmark.type = %i AND bookmark.idViewer = " + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID +
		" LEFT JOIN watchlist ON"
		"    watchlist.idFile=files.idFile AND watchlist.idViewer =  " + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID + "  ";
...
int CVideoDatabase::GetPlayCount(int iFileId)
{
  ...
  try
  {
    ...
    std::string strSQL = PrepareSQL("select playCount from watchlist WHERE idFile=%i AND watchlist.idViewer = " + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID, iFileId);
...
void CVideoDatabase::SetPlayCount(const CFileItem &item, int count, const CDateTime &date)
{
  ...
  try
  {
    ...
    std::string strSQL;
     strSQL =PrepareSQL("SELECT * FROM watchlist where idFile=%i AND watchlist.idViewer ="+CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID, id);
    if( m_pDS->query(strSQL.c_str())) {
		if(m_pDS->num_rows() == 0) {
			m_pDS->close();
			strSQL =PrepareSQL("INSERT INTO watchlist (`idFile`, `idViewer`, `playCount`, `lastPlayed`) VALUES (%i, "+CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID+", NULL, NULL);", id);
    ...
    {
      if (!date.IsValid())
        strSQL = PrepareSQL("update watchlist set playCount=%i,lastPlayed='%s' where idFile=%i AND watchlist.idViewer = "+CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID, count, CDateTime::GetCurrentDateTime().GetAsDBDateTime().c_str(), id);
      else
        strSQL = PrepareSQL("update watchlist set playCount=%i,lastPlayed='%s' where idFile=%i AND watchlist.idViewer = "+CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID, count, date.GetAsDBDateTime().c_str(), id);
    }
    else
    {
      if (!date.IsValid())
        strSQL = PrepareSQL("update watchlist set playCount=NULL,lastPlayed=NULL where idFile=%i AND watchlist.idViewer = "+CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID, id);
      else
        strSQL = PrepareSQL("update watchlist set playCount=NULL,lastPlayed='%s' where idFile=%i AND watchlist.idViewer = "+CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID, date.GetAsDBDateTime().c_str(), id);
    }
  ...
  try
  {
    ...
    if (m_profileManager.GetMasterProfile().getLockMode() != LOCK_MODE_EVERYONE && !g_passwordManager.bMasterUser)
    {
      ...
      if (idContent == VIDEODB_CONTENT_MOVIES)
      {
        ...
        extraField = "watchlist.playCount";
      }
      else if (idContent == VIDEODB_CONTENT_TVSHOWS) //this will not get tvshows with 0 episodes
      {
        ...
        if (StringUtils::EqualsNoCase(type, "tag"))
          extraJoin  = PrepareSQL("JOIN "+ CVideoDatabase::tvShowView+" ON tvshow_view.idShow = tag_link.media_id AND tag_link.media_type='tvshow'");
      }
	  ...
	  extFilter.AppendJoin(PrepareSQL("JOIN %s_view%s %s_view ON %s_link.media_id = %s_view.%s AND %s_link.media_type='%s'", view.c_str(), CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID.c_str(), view.c_str(), type, view.c_str(), view_id.c_str(), type, media_type.c_str()));
      extFilter.AppendJoin(PrepareSQL("JOIN files ON files.idFile = %s_view.idFile", view.c_str()));
	  extFilter.AppendJoin(PrepareSQL("JOIN watchlist ON watchlist.idFile=files.idFile AND watchlist.idViewer = " + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID));
    ...
    else
    {
      ...
      if (idContent == VIDEODB_CONTENT_MOVIES)
      {
        ...
        view_id    = "idMovie";
        media_type = MediaTypeMovie;
        extraField = "count(1), count(watchlist.playCount)";
        extraJoin  = PrepareSQL("JOIN files ON files.idFile = %s_view.idFile JOIN watchlist ON watchlist.idFile=files.idFile AND watchlist.idViewer = " + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID, view.c_str());
      }
      ...
      extFilter.AppendField(extraField);
      extFilter.AppendJoin(PrepareSQL("JOIN %s_link ON %s.%s_id = %s_link.%s_id", type, type, type, type, type));
	  extFilter.AppendJoin(PrepareSQL("JOIN %s_view%s %s_view ON %s_link.media_id = %s_view.%s AND %s_link.media_type='%s'",
		  view.c_str(), CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID.c_str(), view.c_str(), type, view.c_str(), view_id.c_str(), type, media_type.c_str()));
      extFilter.AppendJoin(extraJoin);
...
bool CVideoDatabase::GetMusicVideoAlbumsNav(const std::string& strBaseDir, CFileItemList& items, int idArtist /* = -1 */, const Filter &filter /* = Filter() */, bool countOnly /* = false */)
{
  try
  {
    ...
    std::string strSQL = "select %s from "+CVideoDatabase::musicVideoView;
    Filter extFilter = filter;
...
bool CVideoDatabase::GetYearsNav(const std::string& strBaseDir, CFileItemList& items, int idContent /* = -1 */, const Filter &filter /* = Filter() */)
{
  try
  {
    ...
    if (m_profileManager.GetMasterProfile().getLockMode() != LOCK_MODE_EVERYONE && !g_passwordManager.bMasterUser)
    {
      if (idContent == VIDEODB_CONTENT_MOVIES)
      {
        strSQL = "select movie_view.premiered, path.strPath, watchlist.playCount from " + CVideoDatabase::movieView+ " ";
        extFilter.AppendJoin("join files on files.idFile = movie_view.idFile join path on files.idPath = path.idPath LEFT JOIN watchlist ON watchlist.idFile=movie_view.idFile AND watchlist.idViewer = " + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID);
      }
      else if (idContent == VIDEODB_CONTENT_TVSHOWS)
      {
        strSQL = PrepareSQL("select tvshow_view.c%02d, path.strPath from "+ CVideoDatabase::tvShowView, VIDEODB_ID_TV_PREMIERED);
      ...
      {
        strSQL = "select musicvideo_view.premiered, path.strPath, watchlist.playCount from " + CVideoDatabase::musicVideoView + " ";
        extFilter.AppendJoin("join files on files.idFile = musicvideo_view.idFile join path on files.idPath = path.idPath  LEFT JOIN watchlist ON watchlist.idFile=movie_view.idFile AND watchlist.idViewer = " + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID);
      }
      else
        return false;
    }
    else
    {
      ...
      if (idContent == VIDEODB_CONTENT_MOVIES)
      {
        strSQL = "select movie_view.premiered, count(1), count(watchlist.playCount) from " + CVideoDatabase::movieView +" ";
        extFilter.AppendJoin("join files on files.idFile = movie_view.idFile LEFT JOIN watchlist ON watchlist.idFile=movie_view.idFile AND watchlist.idViewer = " + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID);
        extFilter.AppendGroup("movie_view.premiered");
      }
      else if (idContent == VIDEODB_CONTENT_TVSHOWS)
      {
        strSQL = PrepareSQL("select distinct tvshow_view.c%02d from " + CVideoDatabase::tvShowView, VIDEODB_ID_TV_PREMIERED);
        extFilter.AppendGroup(PrepareSQL("tvshow_view.c%02d", VIDEODB_ID_TV_PREMIERED));
      }

      else if (idContent == VIDEODB_CONTENT_MUSICVIDEOS)
      {
        strSQL = PrepareSQL("select musicvideo_view.c%02d, count(1), count(watchlist.playCount) from " + CVideoDatabase::musicVideoView + " ", VIDEODB_ID_MUSICVIDEO_YEAR);
        extFilter.AppendJoin("join files on files.idFile = musicvideo_view.idFile LEFT JOIN watchlist ON watchlist.idFile=movie_view.idFile AND watchlist.idViewer = " + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID);
...
bool CVideoDatabase::GetSeasonsByWhere(const std::string& strBaseDir, const Filter &filter, CFileItemList& items, bool appendFullShowPath /* = true */, const SortDescription &sortDescription /* = SortDescription() */)
{
  try
  {
    ...
    std::string strSQL = "SELECT %s FROM " + CVideoDatabase::seasonView + " ";
    CVideoDbUrl videoUrl;
...
bool CVideoDatabase::GetMoviesByWhere(const std::string& strBaseDir, const Filter &filter, CFileItemList& items, const SortDescription &sortDescription /* = SortDescription() */, int getDetails /* = VideoDbDetailsNone */)
{
  try
  {
	...
	std::string strSQL = "select %s from " + CVideoDatabase::movieView + "LEFT JOIN watchlist ON watchlist.idFile=movie_view.idFile AND watchlist.idViewer =" + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID + " ";
    std::string strSQLExtra;
...
bool CVideoDatabase::GetEpisodesByWhere(const std::string& strBaseDir, const Filter &filter, CFileItemList& items, bool appendFullShowPath /* = true */, const SortDescription &sortDescription /* = SortDescription() */, int getDetails /* = VideoDbDetailsNone */)
{
  try
  {
    ...
    int total = -1;
    //TODO: Testing
	std::string strSQL = "select %s from " + CVideoDatabase::episodeView + " "; // LEFT JOIN watchlist ON watchlist.idFile = episode_view.idFile AND watchlist.idViewer = " + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID + " ";
    CVideoDbUrl videoUrl;
    std::string strSQLExtra;
    Filter extFilter = filter;
    SortDescription sorting = sortDescription;
...
bool CVideoDatabase::HasSets() const
{
  try
  {
    ...
    m_pDS->query("SELECT movie_view.idSet,COUNT(1) AS c FROM " + CVideoDatabase::movieView +
...
std::string CVideoDatabase::GetContentForPath(const std::string& strPath)
{
  ...
  if (scraper)
  {
    if (scraper->Content() == CONTENT_TVSHOWS)
    {
      ...
      std::string sql = "SELECT COUNT(*) FROM "+ CVideoDatabase::episodeView;
...
bool CVideoDatabase::GetMusicVideosByWhere(const std::string &baseDir, const Filter &filter, CFileItemList &items, bool checkLocks /*= true*/, const SortDescription &sortDescription /* = SortDescription() */, int getDetails /* = VideoDbDetailsNone */)
{
  try
  {
    ...
    int total = -1;
    //TODO: test
	std::string strSQL = "select %s from " + CVideoDatabase::musicVideoView + " ";// LEFT JOIN watchlist ON watchlist.idFile = musicvideo_view.idFile AND watchlist.idViewer = " + CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_databaseVideo.userID +" ";
...
unsigned int CVideoDatabase::GetRandomMusicVideoIDs(const std::string& strWhere, std::vector<std::pair<int,int> > &songIDs)
{
  try
  {
    ...
    std::string strSQL = "select distinct idMVideo from "+ CVideoDatabase::musicVideoView;
    if (!strWhere.empty())
...
void CVideoDatabase::ExportToXML(const std::string &path, bool singleFile /* = true */, bool images /* = false */, bool actorThumbs /* false */, bool overwrite /*=false*/)
{
  ...
  try
  {
    ...
    progress = CServiceBroker::GetGUI()->GetWindowManager().GetWindow<CGUIDialogProgress>(WINDOW_DIALOG_PROGRESS);
    // find all movies
    std::string sql = "select * from "+CVideoDatabase::movieView;
    ...
    sql = "select * from "+CVideoDatabase::musicVideoView+"";
    ...
    while (!m_pDS->eof())
    {
	  ...
	  sql = PrepareSQL("select * from %s where idShow=%i order by strFileName, idEpisode", CVideoDatabase::episodeView.c_str(), tvshow.m_iDbId);
      pDS->query(sql);
