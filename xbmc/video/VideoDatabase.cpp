...
bool CVideoDatabase::createdTVShowView = false;
CStdString CVideoDatabase::musicVideoView;
CStdString CVideoDatabase::tvShowView;
CStdString CVideoDatabase::episodeView;
CStdString CVideoDatabase::movieView;
...
//		musicVideoView = "musicvideoview LEFT JOIN watchlist ON watchlist.idFile=musicvideoview.idFile AND watchlist.idViewer = "+ g_advancedSettings.m_databaseVideo.userID;
		CVideoDatabase::tvShowView = "tvshowview"+ g_advancedSettings.m_databaseVideo.userID+" tvshowview";
		CVideoDatabase::movieView = "movieview"+ g_advancedSettings.m_databaseVideo.userID+" movieview";
		CVideoDatabase::musicVideoView = "musicvideoview"+ g_advancedSettings.m_databaseVideo.userID+" musicvideoview";
		CVideoDatabase::episodeView = "episodeview" + g_advancedSettings.m_databaseVideo.userID+" episodeview";
...
void CVideoDatabase::generateTVShowView() {
//	CLog::Log(LOGERROR,"%s  %s   " + g_advancedSettings.m_databaseVideo.userID,g_advancedSettings.Initialized()?"true":"false" , CVideoDatabase::createdTVShowView?"true":"false" );
...
//	   m_pDS->exec("DROP VIEW IF EXISTS episodeview");
	   CStdString episodeview = PrepareSQL("CREATE  OR REPLACE  VIEW episodeview"+g_advancedSettings.m_databaseVideo.userID+" AS SELECT "
	                                       ...
	                                       "    bookmark.idFile=episode.idFile AND bookmark.type=1 AND bookmark.idViewer =watchList.idViewer "                            , VIDEODB_ID_TV_TITLE, VIDEODB_ID_TV_STUDIOS, VIDEODB_ID_TV_PREMIERED, VIDEODB_ID_TV_MPAA, VIDEODB_ID_TV_BASEPATH, VIDEODB_ID_EPISODE_SEASON);
	   m_pDS->exec(episodeview.c_str());

	 //  CLog::Log(LOGINFO, "create tvshowview");
	 //  m_pDS->exec("DROP VIEW IF EXISTS "+tvShowView);
	 //  CStdString tvshowview = PrepareSQL("CREATE OR REPLACE VIEW "+tvShowView+" AS SELECT "
	 //                                     "  tvshow.*,"
	 //                                     "  path.strPath AS strPath,"
	 //                                     "  path.dateAdded AS dateAdded,"
	 //                                     "  MAX(watchlist.lastPlayed) AS lastPlayed,"
	 ...
	 //                                     "    files.idFile=episode.idFile "
	 //		  	  	  	  	  	  	  	 " LEFT JOIN watchlist ON"
	 //		  		  	  	  	  	     "    watchlist.idFile=episode.idFile AND watchtlist.idViewer = %s"
	 //                                     "GROUP BY tvshow.idShow;" , g_advancedSettings.m_databaseVideo.userID);
	 //  m_pDS->exec(tvshowview.c_str());
...
void CVideoDatabase::GetMusicVideosByArtist(const CStdString& strArtist, CFileItemList& items)
{
  try
  {
    ...
    if (strArtist.IsEmpty())  // TODO: SMARTPLAYLISTS what is this here for???
      strSQL=PrepareSQL("select distinct * from "+CVideoDatabase::musicVideoView+" join artistlinkmusicvideo on artistlinkmusicvideo.idMVideo=musicvideoview.idMVideo join actors on actors.idActor=artistlinkmusicvideo.idArtist");
    else
      strSQL=PrepareSQL("select * from "+CVideoDatabase::musicVideoView+" join artistlinkmusicvideo on artistlinkmusicvideo.idMVideo=musicvideoview.idMVideo join actors on actors.idActor=artistlinkmusicvideo.idArtist where actors.strActor='%s'", strArtist.c_str());
    m_pDS->query( strSQL.c_str() );
...
bool CVideoDatabase::GetMovieInfo(const CStdString& strFilenameAndPath, CVideoInfoTag& details, int idMovie /* = -1 */)
{
  try
  {
    ...
    CStdString sql = PrepareSQL("select * from "+CVideoDatabase::movieView+" where idMovie=%i", idMovie);
    if (!m_pDS->query(sql.c_str()))
      return false;
...
bool CVideoDatabase::GetTvShowInfo(const CStdString& strPath, CVideoInfoTag& details, int idTvShow /* = -1 */)
{
  try
  {
    ...
    CStdString sql = PrepareSQL("SELECT * FROM %s WHERE idShow=%i",CVideoDatabase::tvShowView, idTvShow);
    if (!m_pDS->query(sql.c_str()))
      return false;
...
bool CVideoDatabase::GetEpisodeInfo(const CStdString& strFilenameAndPath, CVideoInfoTag& details, int idEpisode /* = -1 */)
{
  try
  {
    ...
    CStdString sql = PrepareSQL("select * from "+CVideoDatabase::episodeView+" LEFT JOIN watchlist ON watchlist.idFile=episodeview.idFile AND watchlist.idViewer ="+ g_advancedSettings.m_databaseVideo.userID +" where idEpisode=%i",idEpisode);
    if (!m_pDS->query(sql.c_str()))
      return false;
...
bool CVideoDatabase::GetMusicVideoInfo(const CStdString& strFilenameAndPath, CVideoInfoTag& details, int idMVideo /* = -1 */)
{
  try
  {
    ...
    CStdString sql = PrepareSQL("select * from "+CVideoDatabase::musicVideoView+" LEFT JOIN watchlist ON watchlist.idFile=musicvideoview.idFile AND watchlist.idViewer ="+ g_advancedSettings.m_databaseVideo.userID +" where idMVideo=%i", idMVideo);
    if (!m_pDS->query(sql.c_str()))
      return false;
...
bool CVideoDatabase::GetFileInfo(const CStdString& strFilenameAndPath, CVideoInfoTag& details, int idFile /* = -1 */)
{
  try
  {
    ...
    CStdString sql = PrepareSQL("SELECT * FROM files "
                                ...
                                "LEFT JOIN bookmark ON bookmark.idFile = files.idFile AND bookmark.type = %i AND bookmark.idViewer = %d "
    				  	  	  	" LEFT JOIN watchlist ON"
    				  	  	  	"    watchlist.idFile=episode.idFile AND watchlist.idViewer = %d"
                                "WHERE files.idFile = %i", CBookmark::RESUME, g_advancedSettings.m_databaseVideo.userID,g_advancedSettings.m_databaseVideo.userID , idFile);
    if (!m_pDS->query(sql.c_str()))
      return false;
...
void CVideoDatabase::GetBookMarksForFile(const CStdString& strFilenameAndPath, VECBOOKMARKS& bookmarks, CBookmark::EType type /*= CBookmark::STANDARD*/, bool bAppend, long partNumber)
{
  try
  {
    ...
    else
    {
      ...
      CStdString strSQL=PrepareSQL("select * from bookmark where idFile=%i and type=%i and idViewer = "+ g_advancedSettings.m_databaseVideo.userID+" order by timeInSeconds", idFile, (int)type);
      m_pDS->query( strSQL.c_str() );
      while (!m_pDS->eof())
      {
        CBookmark bookmark;
...
void CVideoDatabase::DeleteResumeBookMark(const CStdString &strFilenameAndPath)
{
  ...
  try
  {
    CStdString sql = PrepareSQL("delete from bookmark where idFile=%i and type=%i AND idViewer = "+ g_advancedSettings.m_databaseVideo.userID, fileID, CBookmark::RESUME);
    m_pDS->exec(sql.c_str());
  }
  catch(...)
  {
...
void CVideoDatabase::GetEpisodesByFile(const CStdString& strFilenameAndPath, vector<CVideoInfoTag>& episodes)
{
  try
  {
    CStdString strSQL = PrepareSQL("select * from "+CVideoDatabase::episodeView+" where idFile=%i order by c%02d, c%02d asc", GetFileId(strFilenameAndPath), VIDEODB_ID_EPISODE_SORTSEASON, VIDEODB_ID_EPISODE_SORTEPISODE);
    m_pDS->query(strSQL.c_str());
    while (!m_pDS->eof())
    {
...
void CVideoDatabase::ClearBookMarkOfFile(const CStdString& strFilenameAndPath, CBookmark& bookmark, CBookmark::EType type /*= CBookmark::STANDARD*/)
{
  try
  {
    ...
    double mintime = bookmark.timeInSeconds - 0.5f;
    double maxtime = bookmark.timeInSeconds + 0.5f;
    CStdString strSQL = PrepareSQL("select idBookmark from bookmark where idFile=%i and type=%i and playerState like '%s' and player like '%s' and (timeInSeconds between %f and %f) AND idViewer = "+g_advancedSettings.m_databaseVideo.userID, idFile, type, bookmark.playerState.c_str(), bookmark.player.c_str(), mintime, maxtime);
...
bool CVideoDatabase::GetPlayCounts(const CStdString &strPath, CFileItemList &items)
{
  ...
  try
  {
    ...
    CStdString sql = PrepareSQL(
      ...
      "FROM files"
      "  LEFT JOIN bookmark ON"
      "    files.idFile = bookmark.idFile AND bookmark.type = %i AND bookmark.idViewer = %d"
      " LEFT JOIN watchlist ON"
      "    watchlist.idFile=episode.idFile AND watchlist.idViewer = %d"
      "  WHERE files.idPath=%i", (int)CBookmark::RESUME,g_advancedSettings.m_databaseVideo.userID,g_advancedSettings.m_databaseVideo.userID, pathID);
...
int CVideoDatabase::GetPlayCount(const CFileItem &item)
{
  ...
  try
  {
    ...
    CStdString strSQL = PrepareSQL("select playCount from watchlist WHERE idFile=%i AND watchlist.idViewer = %d", id,g_advancedSettings.m_databaseVideo.userID);
    int count = 0;
  ...
  try
  {
    ...
    if (g_settings.GetMasterProfile().getLockMode() != LOCK_MODE_EVERYONE && !g_passwordManager.bMasterUser)
    {
      if (idContent == VIDEODB_CONTENT_MOVIES)
      {
        ...
        extFilter.fields = PrepareSQL("%s.id%s, %s.str%s, path.strPath, watchlist.playCount", type.c_str(), type.c_str(), type.c_str(), type.c_str());
        extFilter.AppendJoin(PrepareSQL("join %slinkmovie on %s.id%s = %slinkmovie.id%s join "+CVideoDatabase::movieView+" on %slinkmovie.idMovie = movieview.idMovie join files on files.idFile = movieview.idFile join path on path.idPath = files.idPath  LEFT JOIN watchlist ON watchlist.idFile=files.idFile AND watchlist.idViewer = %d",
                                        type.c_str(), type.c_str(), type.c_str(), type.c_str(), type.c_str(), type.c_str(), g_advancedSettings.m_databaseVideo.userID));
      }
      else if (idContent == VIDEODB_CONTENT_TVSHOWS) //this will not get tvshows with 0 episodes
      {
        ...
        extFilter.AppendJoin(PrepareSQL("join %slinktvshow on %s.id%s = %slinktvshow.id%s join "+CVideoDatabase::episodeView+" on %slinktvshow.idShow = episodeview.idShow join files on files.idFile = episodeview.idFile join path on path.idPath = files.idPath",
      ...
      else if (idContent == VIDEODB_CONTENT_MUSICVIDEOS)
      {
        strSQL = "select %s " + PrepareSQL("from %s ", type.c_str());
        extFilter.fields = PrepareSQL("%s.id%s, %s.str%s, path.strPath, watchlist.playCount", type.c_str(), type.c_str(), type.c_str(), type.c_str());
        extFilter.AppendJoin(PrepareSQL("join %slinkmusicvideo on %s.id%s = %slinkmusicvideo.id%s join "+CVideoDatabase::musicVideoView+" on %slinkmusicvideo.idMVideo = musicvideoview.idMVideo join files on files.idFile = musicvideoview.idFile join path on path.idPath = files.idPath LEFT JOIN watchlist ON watchlist.idFile=files.idFile AND watchlist.idViewer = %d",
                                        type.c_str(), type.c_str(), type.c_str(), type.c_str(), type.c_str(), type.c_str() , g_advancedSettings.m_databaseVideo.userID));
      }
      else
        return false;
    ...
    else
    {
      ...
      {
        strSQL = "select %s " + PrepareSQL("from %s ", type.c_str());
        extFilter.fields = PrepareSQL("%s.id%s, %s.str%s, count(1), count(watchlist.playCount)", type.c_str(), type.c_str(), type.c_str(), type.c_str());
        extFilter.AppendJoin(PrepareSQL("join %slinkmovie on %s.id%s = %slinkmovie.id%s join "+CVideoDatabase::movieView+" on %slinkmovie.idMovie = movieview.idMovie join files on files.idFile = movieview.idFile LEFT JOIN watchlist ON watchlist.idFile=files.idFile AND watchlist.idViewer = %d",
                                        type.c_str(), type.c_str(), type.c_str(), type.c_str(), type.c_str(), type.c_str(),g_advancedSettings.m_databaseVideo.userID));
      ...
      else if (idContent == VIDEODB_CONTENT_TVSHOWS)
      {
        ...
        extFilter.AppendJoin(PrepareSQL("join %slinktvshow on %s.id%s = %slinktvshow.id%s join "+CVideoDatabase::tvShowView+" on %slinktvshow.idShow = tvshowview.idShow",
                                        type.c_str(), type.c_str(), type.c_str(), type.c_str(), type.c_str(), type.c_str()));
      }
      else if (idContent == VIDEODB_CONTENT_MUSICVIDEOS)
      {
        strSQL = "select %s " + PrepareSQL("from %s ", type.c_str());
        extFilter.fields = PrepareSQL("%s.id%s, %s.str%s, count(1), count(watchlist.playCount)", type.c_str(), type.c_str(), type.c_str(), type.c_str());
        extFilter.AppendJoin(PrepareSQL("join %slinkmusicvideo on %s.id%s = %slinkmusicvideo.id%s join "+CVideoDatabase::musicVideoView+" on %slinkmusicvideo.idMVideo = musicvideoview.idMVideo join files on files.idFile = musicvideoview.idFile LEFT JOIN watchlist ON watchlist.idFile=files.idFile AND watchlist.idViewer = %d",
                                        type.c_str(), type.c_str(), type.c_str(), type.c_str(), type.c_str(), type.c_str(), g_advancedSettings.m_databaseVideo.userID));
        extFilter.AppendGroup(PrepareSQL("%s.id%s", type.c_str(), type.c_str()));
      }
      else
...
bool CVideoDatabase::GetTagsNav(const CStdString& strBaseDir, CFileItemList& items, int idContent /* = -1 */, const Filter &filter /* = Filter() */, bool countOnly /* = false */)
{
  ...
  try
  {
    ...
    if (idContent == (int)VIDEODB_CONTENT_MOVIES)
      extFilter.AppendJoin("JOIN "+CVideoDatabase::movieView+" ON movieview.idMovie = taglinks.idMedia");
...
bool CVideoDatabase::GetMusicVideoAlbumsNav(const CStdString& strBaseDir, CFileItemList& items, int idArtist /* = -1 */, const Filter &filter /* = Filter() */, bool countOnly /* = false */)
{
  try
  {
    ...
    CStdString strSQL = "select %s from "+CVideoDatabase::musicVideoView+" ";
    Filter extFilter = filter;
    if (g_settings.GetMasterProfile().getLockMode() != LOCK_MODE_EVERYONE && !g_passwordManager.bMasterUser)
    {
  ...
  try
  {
    ...
    if (g_settings.GetMasterProfile().getLockMode() != LOCK_MODE_EVERYONE && !g_passwordManager.bMasterUser)
    {
      if (idContent == VIDEODB_CONTENT_MOVIES)
      {
        ...
        extFilter.fields = "actors.idActor, actors.strActor, actors.strThumb, path.strPath, watchlist.playCount";
        extFilter.AppendJoin(PrepareSQL("join %slinkmovie on actors.idActor = %slinkmovie.id%s join "+CVideoDatabase::movieView+" on %slinkmovie.idMovie = movieview.idMovie join files on files.idFile = movieview.idFile join path on path.idPath = files.idPath LEFT JOIN watchlist ON watchlist.idFile=files.idFile AND watchlist.idViewer = %d",
                                        type.c_str(), type.c_str(), type.c_str(), type.c_str(),g_advancedSettings.m_databaseVideo.userID));
      }
      else if (idContent == VIDEODB_CONTENT_TVSHOWS)
      {
        strSQL = "select %s from actors ";
        extFilter.fields = "actors.idActor, actors.strActor, actors.strThumb, path.strPath";
        extFilter.AppendJoin(PrepareSQL("join %slinktvshow on actors.idActor = %slinktvshow.id%s join "+CVideoDatabase::episodeView+" on %slinktvshow.idShow = episodeview.idShow join files on files.idFile = episodeview.idFile join path on path.idPath = files.idPath",
      ...
      else if (idContent == VIDEODB_CONTENT_EPISODES)
      {
        strSQL = "select %s from actors ";
        extFilter.fields = "actors.idActor, actors.strActor, actors.strThumb, path.strPath, watchlist.playCount";
        extFilter.AppendJoin(PrepareSQL("join %slinkepisode on actors.idActor = %slinkepisode.id%s join "+CVideoDatabase::episodeView+" on %slinkepisode.idEpisode = episodeview.idEpisode join files on files.idFile = episodeview.idFile join path on path.idPath = files.idPath LEFT JOIN watchlist ON watchlist.idFile=files.idFile AND watchlist.idViewer = %d",
                                        type.c_str(), type.c_str(), type.c_str(), type.c_str(),g_advancedSettings.m_databaseVideo.userID));
      }
      else if (idContent == VIDEODB_CONTENT_MUSICVIDEOS)
      {
    ...
    else
    {
      ...
      {
        strSQL ="select %s from actors ";
        extFilter.fields = "actors.idActor, actors.strActor, actors.strThumb, count(1), count(watchList.playCount)";
        extFilter.AppendJoin(PrepareSQL("join %slinkmovie on actors.idActor = %slinkmovie.id%s join "+CVideoDatabase::movieView+" on %slinkmovie.idMovie = movieview.idMovie join files on files.idFile = movieview.idFile LEFT JOIN watchlist ON watchlist.idFile=files.idFile AND watchlist.idViewer = %d",
                                        type.c_str(), type.c_str(), type.c_str(), type.c_str(), g_advancedSettings.m_databaseVideo.userID));
      ...
      {
        strSQL = "select %s " + PrepareSQL("from %slinkepisode, actors, "+CVideoDatabase::episodeView+", files ", type.c_str());
        extFilter.fields = "actors.idActor, actors.strActor, actors.strThumb, count(1), count(watchlist.playCount)";
        extFilter.AppendWhere(PrepareSQL("actors.idActor = %slinkepisode.id%s and %slinkepisode.idEpisode = episodeview.idEpisode and files.idFile = episodeview.idFile LEFT JOIN watchlist ON watchlist.idFile=files.idFile AND watchlist.idViewer = %d",
                                         type.c_str(), type.c_str(), type.c_str(), g_advancedSettings.m_databaseVideo.userID));
        extFilter.AppendGroup("actors.idActor");
      }
      else if (idContent == VIDEODB_CONTENT_MUSICVIDEOS)
      {
        strSQL = "select %s from actors ";
        extFilter.fields = "actors.idActor, actors.strActor, actors.strThumb, count(1), count(watchlist.playCount)";
        extFilter.AppendJoin(PrepareSQL("join %slinkmusicvideo on actors.idActor = %slinkmusicvideo.id%s join "+CVideoDatabase::musicVideoView+" on %slinkmusicvideo.idMVideo = musicvideoview.idMVideo join files on files.idFile = musicvideoview.idFile LEFT JOIN watchlist ON watchlist.idFile=files.idFile AND watchlist.idViewer = %d",
                                        type.c_str(), type.c_str(), type.c_str(), type.c_str(),g_advancedSettings.m_databaseVideo.userID));
        extFilter.AppendGroup("actors.idActor");
      }
      else
  ...
  try
  {
    ...
    if (g_settings.GetMasterProfile().getLockMode() != LOCK_MODE_EVERYONE && !g_passwordManager.bMasterUser)
    {
      if (idContent == VIDEODB_CONTENT_MOVIES)
      {
        ...
        extFilter.AppendJoin(PrepareSQL("join files on files.idFile = movieview.idFile join path on files.idPath = path.idPath LEFT JOIN watchlist ON watchlist.idFile=files.idFile AND watchlist.idViewer = %d",g_advancedSettings.m_databaseVideo.userID));
      }
      else if (idContent == VIDEODB_CONTENT_TVSHOWS)
      {
      ...
      else if (idContent == VIDEODB_CONTENT_MUSICVIDEOS)
      {
        ...
        extFilter.AppendJoin(PrepareSQL("join files on files.idFile = musicvideoview.idFile join path on files.idPath = path.idPath LEFT JOIN watchlist ON watchlist.idFile=files.idFile AND watchlist.idViewer = %d\",g_advancedSettings.m_databaseVideo.userID"));
      }
      else
        return false;
    }
    else
    {
      ...
      if (idContent == VIDEODB_CONTENT_MOVIES)
      {
        ...
        extFilter.AppendJoin(PrepareSQL("join files on files.idFile = movieview.idFile LEFT JOIN watchlist ON watchlist.idFile=files.idFile AND watchlist.idViewer = %d",g_advancedSettings.m_databaseVideo.userID));
      ...
      else if (idContent == VIDEODB_CONTENT_MUSICVIDEOS)
      {
        ...
        extFilter.AppendJoin(PrepareSQL("join files on files.idFile = musicvideoview.idFile LEFT JOIN watchlist ON watchlist.idFile=files.idFile AND watchlist.idViewer = %d",g_advancedSettings.m_databaseVideo.userID));
...
bool CVideoDatabase::GetSeasonsNav(const CStdString& strBaseDir, CFileItemList& items, int idActor, int idDirector, int idGenre, int idYear, int idShow, bool getLinkedMovies /* = true */)
{
  try
  {
    ...
    CStdString strSQL = PrepareSQL("SELECT episodeview.c%02d, "
                                          ...
                                          "seasons.idSeason, "
                                          "count(1), count(watchlist.playCount) "
...
bool CVideoDatabase::GetTvShowsByWhere(const CStdString& strBaseDir, const Filter &filter, CFileItemList& items, const SortDescription &sortDescription /* = SortDescription() */)
{
  try
  {
    ...
    CStdString strSQL = "SELECT %s FROM "+CVideoDatabase::tvShowView;
    CVideoDbUrl videoUrl;
    CStdString strSQLExtra;
    Filter extFilter = filter;
    SortDescription sorting = sortDescription;
...
bool CVideoDatabase::HasSets() const
{
  try
  {
    ...
    m_pDS->query("SELECT movieview.idSet,COUNT(1) AS c FROM "+CVideoDatabase::movieView+" "
...
int CVideoDatabase::GetMusicVideoCount(const CStdString& strWhere)
{
  try
  {
    ...
    CStdString strSQL;
    strSQL.Format("select count(1) as nummovies from "+CVideoDatabase::musicVideoView+" where %s",strWhere.c_str());
    m_pDS->query( strSQL.c_str() );
...
CStdString CVideoDatabase::GetContentForPath(const CStdString& strPath)
{
  ...
  if (scraper)
  {
    ...
    { // check for episodes or seasons.  Assumptions are:
      ...
      CStdString sql = PrepareSQL("select count(1) from "+CVideoDatabase::episodeView+" where strPath = '%s' limit 1", strPath.c_str());
      m_pDS->query( sql.c_str() );
...
unsigned int CVideoDatabase::GetMusicVideoIDs(const CStdString& strWhere, vector<pair<int,int> > &songIDs)
{
  try
  {
    ...
    CStdString strSQL = "select distinct idMVideo from "+CVideoDatabase::musicVideoView+" " + strWhere;
...
bool CVideoDatabase::GetRandomMusicVideo(CFileItem* item, int& idSong, const CStdString& strWhere)
{
  try
  {
    ...
    CStdString strSQL;
    strSQL.Format("select * from "+CVideoDatabase::musicVideoView+" where %s order by RANDOM() limit 1",strWhere.c_str());
    CLog::Log(LOGDEBUG, "%s query = %s", __FUNCTION__, strSQL.c_str());
    // run query
    if (!m_pDS->query(strSQL.c_str()))
      return false;
...
void CVideoDatabase::ExportToXML(const CStdString &path, bool singleFiles /* = false */, bool images /* = false */, bool actorThumbs /* false */, bool overwrite /*=false*/)
{
  ...
  try
  {
    ...
    progress = (CGUIDialogProgress *)g_windowManager.GetWindow(WINDOW_DIALOG_PROGRESS);
    // find all movies
    CStdString sql = "select * from "+CVideoDatabase::movieView+"";
